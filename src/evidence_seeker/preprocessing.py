...
class PreprocessingWorkflow(Workflow):
    ...
    async def _prompt_step(self, ctx: Context, ev: DictInitializedPromptEvent, append_input: bool = False, **kwargs) -> Dict[str, Any]:
    ...
    async def start(
        ...
        ctx.send_event(DescriptiveAnalysisEvent(
            init_data_dict=self.config['pipeline']['preprocessing']['workflow_events'],
    ...
    async def list_descriptive_claims(self, ctx: Context, ev: ListDescriptiveClaimsEvent) -> DescriptiveAnalysisEndEvent:
        ...
        request_dict[ev.result_key] = Claims.model_validate_json(request_dict[ev.result_key])
    ...
    # ) -> StopEvent:
        ...
        num_descriptive_claims = len(request_dict['list_descriptive_claims_event'].claims)
        ...
        await ctx.set(
            "num_claims_to_negate", 
            num_ascriptive_claims + num_descriptive_claims
        )
        for claim_type in ['descriptive', 'ascriptive']:
            for claim in request_dict[f"list_{claim_type}_claims_event"].claims:
                ctx.send_event(NegateClaimEvent(
                    init_data_dict=config['pipeline']['preprocessing']['workflow_events'],
                    request_dict=request_dict,
    ...
    @step(num_workers=10)
    async def negate_claim(self, ctx: Context, ev: NegateClaimEvent) -> CollectCheckedClaimsEvent:
    ...
    @step
    async def collect_checked_claims(self, ctx: Context, ev: CollectCheckedClaimsEvent) -> StopEvent:
        claims_to_collect = await ctx.get("num_claims_to_negate")
        results = ctx.collect_events(ev, [CollectCheckedClaimsEvent] * claims_to_collect)
        if results is None:
            return None
