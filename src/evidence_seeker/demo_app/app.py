...
def setup_from_app_config() -> EvidenceSeeker | None:
    global APP_CONFIG  
    configs = {"preprocessing_config_file": APP_CONFIG.preprocessing_config_file, 
...
def describe(ev_result: EvidenceSeekerResult) -> str:
    ...
    env = Environment(loader=FileSystemLoader(os.path.dirname(APP_CONFIG.result_template_file)))
    result_template = env.get_template(os.path.basename(APP_CONFIG.result_template_file))
...
def log_feedback(clicked_button, last_result : EvidenceSeekerResult):
    ...
    last_result.feedback["binary"] = None if last_result.feedback["binary"] == choice else choice
    logger.log('INFO',f"{last_result.feedback['binary']} feedback on results")
    return gr.Button(variant="primary" if last_result.feedback["binary"] else "secondary"), gr.Button(variant="secondary")
...
async def check(statement : str, last_result : EvidenceSeekerResult):
    ...
    logger.log("INFO", f"Result of statement '{statement}' checked (request {last_result.request_uid})")
...
def log_result(last_result : EvidenceSeekerResult):
    ...
    ts = datetime.strptime(last_result.request_time, '%Y-%m-%d %H:%M:%S UTC').strftime('%Y_%m_%d')
    ...
    if RUNS_ON_SPACES:
        ...
        filepath = "/".join([result_dir, fn]) if subdir == "" else "/".join([result_dir, subdir, fn])
        assert APP_CONFIG.github_token is not None and APP_CONFIG.github_token in os.environ.keys()
        ...
        try:
            ...
            repo.update_file(filepath, f"Update result ({last_result.request_uid})", content, c.sha)
        except UnknownObjectException:
            repo.create_file(filepath, f"Upload new result ({last_result.request_uid})", content)
        return
    else:
    ...
    def renderApp(v : bool, a : tuple[bool, bool]):
        if v and a[0]:
            gr.Markdown("""
                ...
                """, key="intro")
            with gr.Row():
                statement = gr.Textbox(value=f"",label="Zu prüfende Aussage:", interactive=True, scale=10,lines=3,key="statement")
            ...
            result = gr.Markdown("",min_height=80,key="results", show_copy_button=True)
            with gr.Column():
                feedback.render()
                with gr.Row():
                    good.render()
            ...
            check_btn.click(deactivate, None, [check_btn,good,bad,feedback]).then((lambda: "### Aussage wird geprüft... Dies könnte ein paar Minuten dauern."), None, result).then(check, [statement, last_result], [result,last_result]).then(logging, last_result, None).then(reactivate, [check_btn,statement], [feedback,check_btn,good,bad])
            good.click(log_feedback, [good,last_result], [good,bad]).then(logging, [last_result], None)
            bad.click(log_feedback, [bad,last_result], [bad,good]).then(logging, [last_result], None)
            ...
            consent_box = gr.Checkbox(False, label=APP_CONFIG.consent_text, info="**Einwilligung zur Datenweiterverarbeitung (Optional)**")
            ...
            gr.Markdown("""
                ...
                """)
            box = gr.Textbox(label="Bitte für Zugriff Passwort eingeben", autofocus=True, type="password", submit_btn=True)
