...
For using a PostgreSQL database, you have to configure several parameters, besides setting `use_postgres` to `True` as mentioned above. A short snippet of an example configuration of these parameters can be found below as well as [here](./example_advanced_index_operations.qmd#postgresql-vector-store-support). The following describes the parameters and their functionality in detail:
...
Before executing the pipeline, make sure your PostgreSQL server is running at the specified address. For information on how to set up a PostgreSQL server and database, refer to the official documentation (here)[https://www.postgresql.org/docs/]
